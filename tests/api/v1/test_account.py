...
async def test_create_and_get_account(client):
    ...
    get_response = client.get(f"/api/v1/accounts/{created_account_id}")
...
@pytest.mark.asyncio
async def test_create_same_account(client):
    # Create same account
    response = client.post("/api/v1/accounts", json={"name": "Test Account"})
    assert response.status_code == 409

@pytest.mark.asyncio
async def test_create_update_delete_account(client):
...
@pytest.mark.asyncio
async def test_get_wrong_account(client):
    response = client.get("/api/v1/accounts/0")
    assert response.status_code == 404

@pytest.mark.asyncio
async def test_update_wrong_account(client):
    response = client.put("/api/v1/accounts/0", json={"name": "Test Account 2 - Updated"})
    assert response.status_code == 404

@pytest.mark.asyncio
async def test_delete_wrong_account(client):
